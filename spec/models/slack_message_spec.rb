...
RSpec.describe SlackMessage do
  ...
  it "#new_product_order" do
    ...
    expect(SlackMessage).to receive(:message).with(
      "<http://localhost:5000/ops/buyers/1|Buyer Buyer> from Organisation Name wants to buy <http://localhost:5000/cloud/applications-software/products/1|Product name>. <http://localhost:5000/ops/product-orders/1|View product order>."
    )
